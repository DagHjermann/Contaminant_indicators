...
dat_bluemussel_medians <- dat_bluemussel_raw %>%
...
ggplot(dat_bluemussel_medians, aes(MYEAR, ymin = VALUE_lo, ymax = VALUE_up)) +
  geom_errorbar() +
...
dat_bluemussel_lastyear_1 <- dat_bluemussel_medians %>%
...
nrow(dat_bluemussel_lastyear_1)

dat_bluemussel_lastyear_2 <- dat_bluemussel_lastyear_1 %>%
  left_join(
    lookup_thresholds_bluemussel %>% select(PARAM, EQS_threshold, Mattrygghet_threshold),
    by = "PARAM") %>%
  left_join(
    lookup_proref_bluemussel %>% select(PARAM, PROREF),
    by = "PARAM") %>%
  mutate(
    EQS = case_when(
...
nrow(dat_bluemussel_lastyear_2)
...
if (FALSE){
  ...
  dat_bluemussel_trends <- get_trendslope_all(dat_bluemussel_medians)
  
  # 86 percent of analyses succeeded (18 of a total of 21)
  
  saveRDS(dat_bluemussel_trends, "Data/15_dat_bluemussel_trends.rds")
  write.csv(dat_bluemussel_trends, "Data/15_dat_bluemussel_trends.csv")
  
}

dat_bluemussel_trends <- readRDS("Data/15_dat_bluemussel_trends.rds")
...
n1 <- nrow(dat_bluemussel_lastyear_2)

dat_indicator_bluemussel <- dat_bluemussel_lastyear_2 %>%
  left_join(dat_bluemussel_trends, by = c("STATION_CODE", "PARAM")) %>%
  left_join(lookup_coord %>% select(STATION_CODE, Long, Lat), by = "STATION_CODE") %>%
  rename(LATITUDE = Lat, LONGITUDE = Long) %>%
  select(-Mattrygghet_threshold, -EQS_threshold)

n2 <- nrow(dat_indicator_bluemussel)

if (n2 > n1)
  stop("Number of rows increased, check left joins!")

dat_indicator_draft2 <- dat_indicator_draft1 %>%
  bind_rows(dat_indicator_bluemussel)

names(dat_indicator_draft1)
names(dat_indicator_draft2)
