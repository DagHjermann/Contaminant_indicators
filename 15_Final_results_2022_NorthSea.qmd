...
dat_trend_all_02 <- dat_trend_all_01 %>%
  mutate(
    Basis_selected = case_when(
      LATIN_NAME == "Gadus morhua" & PARAM %in% "HG" ~ "WWa",
      TRUE ~ "WW")
  ) %>%
  filter(
    Trend_type == "short",
...
* Using lower bound for assessment of levels  
    - which equals zero for single substances under LOQ  

* NOTE: modifgying how to treat <LOQ values in relation to 
    - e.g. since 2017, HCB in blue mussel has always been under 0.5, while EQS is 10, so they should be marked as EQS = 1  
* Possible rule: 
    - for last year with data: if LOQ < EQS and data are under LOQ, mark station as "under EQS"  
    - for last year with data: if LOQ > EQS and data are under LOQ, but some earlier year is > LOQ: mark station depending on that value  
    - for last year with data: if LOQ > EQS and data are under LOQ, and no earlier year is > LOQ: mark station as "unknown" (or "over EQS"?)  
* Start by setting status year by year, following these codes for EQS and Mattrygghet:
    - 1 = under threshold
    - 2 = over threshold
    - 0 = uncertain  
...
dat_medians_1 <- dat_all %>%
  ...
  summarise(
    Conc_lo = median(VALUE_lo),
    Conc_up = median(VALUE_up),
    .groups = "drop"
  ) 

dat_medians_1b <- dat_medians_1 %>%
  bind_rows(
    dat_medians_hi %>% select(PARAM, STATION_CODE, LATIN_NAME, TISSUE_NAME, MYEAR, Conc_lo, Conc_up)
  )
...
dat_medians_4 <- dat_medians_3 %>%
  mutate(
    EQS = case_when(
      Conc_up/EQS_threshold <= 1 ~ 1,
      Conc_up/EQS_threshold > 1 & Conc_lo/EQS_threshold > 1 ~ 2,
      Conc_up/EQS_threshold > 1 & Conc_lo/EQS_threshold <= 1 ~ 0),
    Mattrygghet = case_when(
      Conc_up/Mattrygghet_threshold <= 1 ~ 1,
      Conc_up/Mattrygghet_threshold > 1 & Conc_lo/Mattrygghet_threshold > 1 ~ 2,
      Conc_up/Mattrygghet_threshold > 1 & Conc_lo/Mattrygghet_threshold <= 1 ~ 0),
...
#
# MODIFIED CODE - NOT FINISHED
#

dat_status_EQS_1 <- dat_medians_4 %>%
  filter(
    STATION_CODE %in% unique(lookup_old_mussel$STATION_CODE),
    PARAM %in% unique(lookup_old_mussel$PARAM),
    MYEAR >= (selected_year - 10)) %>%
  group_by(STATION_CODE, PARAM) %>%
  mutate(
    Last_year_status_known = max(MYEAR[EQS %in% 1:2]),
    No_years_status_known = sum(EQS %in% 1:2),
    Year_select = case_when(
      No_years_status_known > 0 ~ Last_year_status_known,
      No_years_status_known == 0 ~ max(MYEAR))
    ) %>% 
  ungroup()

dat_status_EQS <- dat_status_EQS_1 %>%
  # Pick only that year
  filter(MYEAR == Year_select) %>%
  select(STATION_CODE, PARAM, MYEAR, Conc_lo, Conc_up, EQS) %>%
  rename(
    EQS_year = MYEAR,
    EQS_conc_lo = Conc_lo,
    EQS_conc_up = Conc_up
...
for (param in unique(dat_indicator$PARAM)){
  # param <- "CD"
  gg <- dat_indicator %>%
    filter(PARAM == param & LATIN_NAME == "Mytilus edulis") %>%
    mutate(
      EQS = factor(EQS),
      Mattrygghet = factor(Mattrygghet)) %>%
    ggplot(aes(Long, Lat)) +
    annotation_map(very_simple_map, fill = "grey80") +
    geom_point(aes(color = Mattrygghet), size = 8) +
    geom_point(aes(color = EQS), size = 5) +
    scale_color_manual(values = c("lightblue", "pink2")) +
    coord_fixed(ratio = 2, xlim = c(4.5, 11.5), ylim = c(57.8, 62.5)) +
    geom_point(aes(shape = Trend_string)) +
    scale_shape_manual(
      values = c("Decreasing" = 6,
                 "Increasing" = 2,
                 "No change" = 0,
                 "No trend" = 4)) +
    labs(title = param) +
