---
title: "Get levels/trends fro bluemussel, 2021"
...
fn_export <- "Data_export/MytiEdu_2020_ver1.csv"
overwrite <- FALSE
...
sel_records <- with(data_xl, PARAM %in% sel_param & 
                             ...
                             Basis == "WW" & !is.na(`PROREF-class this year`))
...
df_q95 <- data_xl_sel[data_xl_sel$Basis %in% "WW", c("PARAM", "LATIN_NAME", "TISSUE_NAME", "PROREF", "EQS-threshold last year")] %>%
  ...
  summarise(Q95 = first(`EQS-threshold last year`), EQS_threshold = first(`EQS-threshold last year`))
...
if (overwrite | !file.exists(fn_export)){
  ...
  cat("File", fn_export, "exists, data not saved.\nSet 'overwrite <- TRUE' to overwrite file.\n")
}
