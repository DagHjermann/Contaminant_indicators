...
get_medians_all_stations <- function(...){
  df <- get_plot_data(...)
  # head(df)
  # xtabs(~STATION_CODE + YEAR, df)
  # df$YEAR_f <- factor(df$YEAR, levels = yrs)
...
get_medians_for_regression <- function(mediandata, station, yrs){
  ...
  sel <- mediandata$STATION_CODE %in% station & mediandata$YEAR %in% yrs
  mediandata[sel,]
  }
...
#
# The data frame must have the variable "YEAR"
...
select_for_regr_b <- function(sel_object){
  ...
  # We let YEARS stay unchanged and change YEAR_regr
  df_med_st$YEAR_regr <- df_med_st$YEAR
  df_med_st[sel_ts,]
  ...
  df_med_st$YEAR_regr[index_after_last_over_LOQ] <- df_med_st$YEAR[index_last_over_LOQ] 
...
GAM_trend_analysis <-  function(dataset){
  ...
  mgam <- gam(y ~ s(x, k = max_df + 1), data = dataset[sel,])
...
# for testing
# df_med_st <- data.frame(YEAR = 2001:2010, Median = seq(10,1), Over_LOQ = c(0,0,0,0,0,1,1,0,1,0))  # rule 1
# df_med_st <- data.frame(YEAR = 2001:2010, Median = seq(10,1), Over_LOQ = c(0,0,1,1,0,1,0,1,1,0))  # rule 2
...
calc_models <- function(obj, var_x = "YEAR_regr", var_y = "Median", gam = FALSE){
...
# Based on 'models_yr', but it uses the entire input data for regression
calc_models_gam <- function(obj, var_x = "YEAR_regr", var_y = "Median", gam = FALSE, log = TRUE){
  data <- obj$df_med_st[obj$sel_ts,]
  data_pick <- data[, c(var_x, var_y)]
  if (log)
    data_pick[,2] <- log(data_pick[,2])
...
data_for_excel <- function(df_med, station, yrs = 1990:2016){
  # For output to table
  df_med_st <- df_med[df_med$STATION_CODE %in% station & df_med$YEAR %in% yrs,]
  df_med_st$YEAR_f <- factor(df_med_st$YEAR, levels = yrs)
  df_med_st[, c("YEAR_f", "Median")] %>%
    spread(YEAR_f, Median, drop = FALSE)
  }
...
plot_object <- function(obj, xlab = "", ylab = "Median conc"){
  plot(Median~YEAR, obj$df_med_st, log = "y", xlab = xlab, ylab = ylab)
  lines(exp(Fit_lin)~YEAR, obj$df_med_st, col = colors[1])
  lines(exp(Fit_nonlin)~YEAR, obj$df_med_st, col = colors[2])
  }
...
# The input data should have the variables YEAR, Median and Over_LOQ
# Also,
...
plot_models_one_station <- function(result_object, logdata = FALSE, logscale = TRUE, title = ""){
  if (logdata){
    plot(log(Median)~YEAR, result_object$data_object$df_med_st, type = "n", main = title)
    points(log(Median)~YEAR, result_object$data_object$df_med_st[result_object$data_object$sel_ts,], pch = 21, bg = "blue3")
    points(log(Median)~YEAR, result_object$data_object$df_med_st[!result_object$data_object$sel_ts,], pch = 21, bg = "indianred1")
    lines(Fit_lin~YEAR, result_object$data_object$df_med_st, lty = "dashed", col = colors[1])
    lines(Fit_nonlin~YEAR, result_object$data_object$df_med_st, lty = "dotted", col = colors[2])
  } else if (!logdata){
    if (!logscale){
      plot(Median~YEAR, result_object$data_object$df_med_st, main = title)
    } else {
      plot(Median~YEAR, result_object$data_object$df_med_st, main = title, log = "y")
    }
    points(Median~YEAR, result_object$data_object$df_med_st[result_object$data_object$sel_ts,], pch = 21, bg = "blue3")
    points(Median~YEAR, result_object$data_object$df_med_st[!result_object$data_object$sel_ts,], pch = 21, bg = "indianred1")
    lines(exp(Fit_lin)~YEAR, result_object$data_object$df_med_st, lty = "dashed", col = colors[1])
    lines(exp(Fit_nonlin)~YEAR, result_object$data_object$df_med_st, lty = "dotted", col = colors[2])
    }
  }
...
plot_models_one_station_gg <- function(result_object, title = "", log = TRUE){
  ...
  if (log){
    gg <- ggplot(result_object$data_object$df_med_st, aes(YEAR, log(Median)))
  } else {  
    gg <- ggplot(result_object$data_object$df_med_st, aes(YEAR, Median))
  }
  gg <- gg + 
  ...
  list(result_object = result_object,
       statistics = 
         cbind(
          ...
          result_stat,
          N_data = sum(df_med$YEAR %in% yrs),                # this is the total number of years with data, regardless of data are over LOQ
