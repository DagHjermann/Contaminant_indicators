...
df_indicator2 <- df_indicator %>%
  left_join(df_median_2022, multiple ="any") %>% 
  left_join(df_nifes_cod3, multiple ="any") %>% 
  left_join(niva_stations, multiple ="any") %>% 
  left_join(df_nifes_stations, multiple = "any") 
  

df_indicator2 <- df_indicator2 %>% 
  select(-MYEAR, -VALUE_lo, -VALUE_up, -Year, -Posisjon, -Parameter, -Organ)
...
df_indicator2 <- left_join(
  df_indicator2, 
  subset(df_limits, select = c(PARAM, LATIN_NAME, NIVA_CODE)), 
